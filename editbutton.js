...
function makeMasterEditButton() {
    var btn = document.createElement('a');
    btn.textContent = 'Edit';
    btn.classList.add('item-edit'); // give button the class "item-delete"
    btn.setAttribute('href', '#');
    return btn;
}
...
if (userInput !== null && saveButton !== null && resultEl !== null) {
    ...
    saveButton.addEventListener('click', function (event) {
        ...
        if (submitted !== '') {
            ...
            // Make a edit button element and append it to p element
            itemEditButton = makeMasterEditButton();
            newNode.insertAdjacentElement('afterbegin', itemEditButton);
    ...
    resultEl.addEventListener('click', function(event) {
        event.preventDefault();
        var attr = event.target.classList.contains('item-delete');
        if(attr) {
    ...
    resultEl.addEventListener('click', function(event) {
        event.preventDefault();
        var attr = event.target.classList.contains('item-edit');
        var editText = resultEl.getElementsByTagName('p')[0].childNodes[2].nodeValue;
        if(attr) {
            resultEl.removeChild(event.target.parentNode);
